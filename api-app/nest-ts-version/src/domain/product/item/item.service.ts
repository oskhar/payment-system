...
export class ItemService {
  ...
  async findAll(branchId: number, filterData: FilterDataDto) {
    ...
    const result = await Promise.all(
      items.map(async (item) => ({
        ...item,
        price: item.item_units.find(
          (unit) => unit.unit_id === item.base_unit.id,
        )?.price,
        cost: item.item_units.find((unit) => unit.unit_id === item.base_unit.id)
          ?.cost,
        stock: await this.getAccumulatedStock.execute(branchId, item.id),
      })),
    );
  ...
  async findUnit(id: number) {
    const unit = await this.itemUnitRepository.find({
      where: { item_id: id },
      relations: ['unit'],
    });
