...
<template>
    ...
    <VCardText>
      <VForm @submit.prevent="fetchItems">
        <VRow class="d-flex justify-center align-center">
          <VCol cols="12" md="6">
            <div class="d-flex">
              <VTextField id="search" v-model="searchQuery" placeholder="Temukan" variant="solo" density="comfortable"
                hide-details class="flex-grow-1" @keydown.enter="fetchItems" />
              <VBtn class="ml-3" icon="ri-search-line" type="submit" aria-label="Cari Item" />
            </div>
          </VCol>
        </VRow>
      </VForm>
  ...
  <VCard class="mb-10">
    <VCardTitle class="d-flex justify-space-between align-center pa-5">
      ...
      <VBtn prepend-icon="ri-add-line" color="primary" data-testid="add-item-button" @click="dialogAddItem = true">
        Tambah Item
      </VBtn>
    </VCardTitle>
    ...
    <div class="pb-5 px-5">
      <VRow>
        <VCol v-if="isLoadingItems" cols="12" class="text-center py-16">
          <VProgressCircular indeterminate color="primary" />
          <p class="text-medium-emphasis mt-4">Memuat data item...</p>
        </VCol>

        <template v-else-if="filteredItems.length > 0">
          <VCol v-for="item in filteredItems" :key="item.id" cols="12" sm="6" md="4" lg="3" class="mb-4">
            <ShoppingCard :item="item" @updated="onItemUpdated" />
          </VCol>
        </template>

        <VCol v-else cols="12" class="text-center py-16">
          <VIcon icon="ri-inbox-2-line" size="50" class="text-disabled mb-4" />
    ...
    <div v-if="filteredItems.length > 0" class="text-center pb-5">
      <VPagination v-model="pagination.currentPage" :length="pagination.totalPages" :total-visible="5" rounded="circle"
        @update:model-value="fetchItems" />
    </div>
  </VCard>

  <!-- [UBAH] Dialog Tambah Item disesuaikan -->
  <VDialog v-model="dialogAddItem" max-width="800" persistent>
    <VCard>
      <VForm @submit.prevent="submitForm">
        <VCardTitle class="pa-4 d-flex justify-space-between align-center">
          <span>Tambah Item Baru</span>
          <VBtn icon="ri-close-line" variant="text" @click="closeAndResetDialog" />
        </VCardTitle>
        ...
        <VCardText class="pb-0">
          <VRow>
            ...
            <VCol cols="12" md="6">
              <VTextField v-model="formAddItem.name" label="Nama Item" class="mb-4" required
                data-testid="form-item-name" />

              <div class="d-flex align-start ga-2 mb-4">
                <VSelect v-model="selectedCategories" label="Kategori" :items="categories" item-title="name"
                  item-value="id" multiple chips clearable closable-chips class="flex-grow-1">
                  <template #prepend-item>
                    <VListItem title="Pilih Semua" @click="toggleSelectAllCategories">
                      <template #prepend>
                        <VCheckbox :model-value="isAllCategoriesSelected" readonly class="mr-2" />
                      </template>
                    </VListItem>
                    <VDivider class="mt-2" />
                  </template>
                </VSelect>
                <VBtn icon="ri-add-line" aria-label="Tambah Kategori Baru" title="Tambah Kategori Baru"
                  @click="addCategory" />
              </div>

              <VFileInput id="file-input" label="Upload Gambar (Opsional)" accept="image/*" class="mb-4"
                @change="handleFileUpload" />

              <VTextField v-model="formAddItem.barcode" label="Barcode Item (Opsional)" class="mb-4" />

              <VTextarea v-model="formAddItem.description" label="Deskripsi (Opsional)" rows="3" />
            </VCol>
            ...
            <VCol cols="12" md="6">
              ...
              <VRadioGroup v-model="formAddItem.base_unit_id" class="w-100">
                <div v-for="(unit, index) in formAddItem.units" :key="index" class="unit-row mb-4">
                  ...
                  <VDivider v-if="index < formAddItem.units.length - 1" class="mt-4" />
                </div>
              </VRadioGroup>
        ...
        <VCardActions class="pa-4 mt-4 d-flex justify-end">
          <VBtn color="secondary" @click="closeAndResetDialog">
            Batal
          </VBtn>
          <VBtn type="submit" variant="elevated" color="primary" :loading="isSubmitting" :disabled="isSubmitting"
